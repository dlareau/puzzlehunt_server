...
def download_pdf(directory, filename, url):
    ...
    call(["wget", url, "-O", file_str], stdout=FNULL, stderr=STDOUT, shell=True)
    with open(file_str, "rb") as f:
        num_pages = PdfFileReader(f).getNumPages()
    call(["convert", "-density", "200", file_str, directory + "/" + filename + ".png"], stdout=FNULL, stderr=STDOUT, shell=True)
    FNULL.close()
    return num_pages
